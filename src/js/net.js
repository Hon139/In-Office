export class SocketNet {
  constructor(url) { this.url = url; this.sock = null; this.uid = null; this.cb = null; }
  ...
  onEvents(cb) { this.cb = cb; }
  sendState(partial) { this.sock?.emit("state", partial); }
  disconnect() { this.sock?.emit("leave"); this.sock?.disconnect(); }
}
